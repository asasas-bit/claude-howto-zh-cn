...
> 📚 **学习版说明**：这个文件是 `.claude/commands/sync-upstream.md` 的讲解副本。
> 工作原理一样的——Claude Code 启动时会扫描 `.claude/commands/` 目录，把你输入 `/sync-upstream` 对应的这个 Markdown 文件当指令执行。
> 本目录（`学习笔记/D-command/`）只是**学习存档**，不会生效；真正的命令放在 `.claude/commands/` 下。

## 🧠 这个命令解决什么问题

每次上游（`lhfer/claude-howto-zh-cn`）更新，你都要手动做：
`fetch → 看新内容 → merge → push`，非常重复。
...
Claude 会替你完成 fetch、展示新内容、等你确认后 merge，问你是否 push。
...
> ✨ **技巧**：`!` 反引号包起来的命令，会在 Claude 读取这个文件时**真的执行**，把输出注入到指令里。
> 这样 Claude 一开始就知道当前状态，不用再手动查一遍。
